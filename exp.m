...
% tau_ind  = 1200;
% sigma    = 0.01;
% [x1, x2] = signal_generator(time_ind, tau_ind, period, sigma);
...
% tau_cs = estimate_lag(x1, x2, Fs, low_freq, high_freq, 1, tau_ind);
...
acc      = 50;           % The accuracy of the downsampling
...
xcorr_tau = tau0_ind * Ts * ones(1, length(tau_list)); % xcorr tau over downsampling rate
error     = abs(real_tau - tau_list);         % error over downsampling rate

f = figure; 
% subplot(2,1,1); 
plot(x_axis, tau_list, 'g', x_axis, xcorr_tau, 'r'); xlabel('downsampling rate(%)'); ylabel('Tau (s)');
